...
class IDPMetadataReader{
	...
	public function __construct(DOMNode $xml = NULL){
        ...
        $entitiesDescriptor = SAMLSPUtilities::xpQuery($xml, './saml_metadata:EntitiesDescriptor');

        if(!empty($entitiesDescriptor))
            $entityDescriptors = SAMLSPUtilities::xpQuery($entitiesDescriptor[0], './saml_metadata:EntityDescriptor');
        else
            $entityDescriptors = SAMLSPUtilities::xpQuery($xml, './saml_metadata:EntityDescriptor');

        foreach ($entityDescriptors as $entityDescriptor) {
            $idpSSODescriptor = SAMLSPUtilities::xpQuery($entityDescriptor, './saml_metadata:IDPSSODescriptor');
...
class IdentityProviders{
	...
	public function __construct(DOMElement $xml = NULL){
        ...
        $idpSSODescriptor = SAMLSPUtilities::xpQuery($xml, './saml_metadata:IDPSSODescriptor');
        ...
        $info = SAMLSPUtilities::xpQuery($xml, './saml_metadata:Extensions');
	...
	private function parseInfo($xml){
		$displayNames = SAMLSPUtilities::xpQuery($xml, './mdui:UIInfo/mdui:DisplayName');
	...
	private function parseSSOService($xml){
		$ssoServices = SAMLSPUtilities::xpQuery($xml, './saml_metadata:SingleSignOnService');
	...
	private function parseSLOService($xml){
		$sloServices = SAMLSPUtilities::xpQuery($xml, './saml_metadata:SingleLogoutService');
	...
	private function parsex509Certificate($xml){
		foreach ( SAMLSPUtilities::xpQuery($xml, './saml_metadata:KeyDescriptor') as $KeyDescriptorNode ) {
	...
	private function parseSigningCertificate($xml){
		$certNode = SAMLSPUtilities::xpQuery($xml, './ds:KeyInfo/ds:X509Data/ds:X509Certificate');
		$certData = trim($certNode[0]->textContent);
		$certData = str_replace(array ( "\r", "\n", "\t", ' '), '', $certData);
		if(!empty($certNode))
			array_push($this->signingCertificate, SAMLSPUtilities::sanitize_certificate( $certData ));
	}
